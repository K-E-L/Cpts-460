...
/* set Supervisor stack */
  ...
  LDR r0, =proc           // r0->proc0
  LDR r1, =procsize       // r1->procsize
  LDR r2,[r1,#0]          // r2 = procsize
  ADD r0, r0, r2          // r0-> high end of proc0
  MOV sp, r0              // sp-> high end of proc0
...
/* Enable IRQs */
  BIC r0, r0, #0x80
/* go back in SVC mode with IRQ enabled*/
  MSR cpsr, r0
...
// disable IRQ interrupts : r0 is FREE for use
  ...
  bl	scheduler  // call scheduler() in C

  LDR r0, =running
  LDR r1, [r0,#0]     // r1->runningPROC
  lDR sp, [r1,#4]     // restore running->ksp, SVC mode, IRQ interrupts on
...
lock: // mask out IRQ interrupts, disbable IRQ interrupts
  MRS r0, cpsr
  ORR r0, r0, #0x80   // set bit means MASK off IRQ interrupt 
  MSR cpsr, r0
  mov pc, lr	

unlock: // unmask IRQ interrupts, enable IRQ interrupts
  MRS r0, cpsr
  BIC r0, r0, $0x80
  MSR cpsr, r0
  mov pc, lr	
